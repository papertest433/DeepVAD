...
| **ROUGE-L** | Longest Common Subsequence (LCS) | Sentence-level in-order word overlap | ![ROUGE-L](https://latex.codecogs.com/svg.image?\Large%20\text{ROUGE-L}=F_{\text{lcs}}=%20\frac{(1+\beta^2)R_{\text{lcs}}P_{\text{lcs}}}{R_{\text{lcs}}+\beta^2P_{\text{lcs}}}),%20\quad%20R_{\text{lcs}}=%20\frac{\text{LCS}(X,Y)}{m},%20P_{\text{lcs}}=%20\frac{\text{LCS}(X,Y)}{n}) |
